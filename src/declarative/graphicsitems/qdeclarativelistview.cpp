...
public:
    FxListItem(QDeclarativeItem *i, QDeclarativeListView *v) : item(i), section(0), view(v) {
        ...
        item->connect(item, SIGNAL(visibleChanged()), view, SLOT(visibleChanged()));
    }
    ...
    {
        item->disconnect(item, SIGNAL(visibleChanged()), view, SLOT(visibleChanged()));
    }
...
void QDeclarativeListViewPrivate::updateAverage()
{
    ...
    for (int i = 0; i < visibleItems.count(); ++i) {
        if (visibleItems.at(i)->isVisible()) {
...
void QDeclarativeListView::visibleChanged()
{
    Q_D(QDeclarativeListView);
    ...
    if (d->requestedIndex >= 0 || d->currentItem == 0)
        return;
